...
import { Package, AlertTriangle, Truck, Plus,UserPlus, Search, Filter } from 'lucide-react';
    ...
    >
      ...
      <motion.div variants={itemVariants}>
        ...
        <p className="text-sm text-gray-500 mt-1">
          Track stock levels, empty bottles, and vendor relationships
        </p>
      </motion.div>
        ...
        >
          ...
          >
            <div className="px-6 py-4 border-b border-gray-100">
              <h2 className="text-base font-semibold text-gray-900">Refill Stock Tracking</h2>
              <p className="text-xs text-gray-400 mt-0.5">19L refill inventory and demand</p>
            </div>
            <InventoryTable data={refillData} showPredicted={true} />
          </motion.div> */}
          ...
          >
            ...
            <div className="space-y-3">
              {waterUsagePrediction.map((row) => {
                const maxVal = Math.max(...waterUsagePrediction.map((r) => Math.max(r.actual, r.predicted)));
                    ...
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <div className="flex items-center gap-4 text-xs">
                        <span className="text-gray-500">
                          Actual: <span className="font-semibold text-gray-700">{row.actual.toLocaleString()}</span>
                        </span>
                        <span className="text-gray-500">
                          Predicted: <span className="font-semibold text-blue-600">{row.predicted.toLocaleString()}</span>
                        </span>
                      </div>
                    </div>
          ...
          >
            <div className="px-6 py-4 border-b border-gray-100 flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400 mt-0.5">{lowStockAlerts.length} items require restocking</p>
              </div>
            </div>
        ...
        <div className="space-y-6">
          ...
          >
            <div className="px-6 py-4 border-b border-gray-100">
              ...
              <p className="text-xs text-gray-400 mt-0.5">Returned empty bottles awaiting refill across branches</p>
            </div>
          ...
          >
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition-shadow duration-200">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <p className="text-xs text-gray-400 font-medium">Total Empty Bottles</p>
                  <p className="text-xl font-bold text-gray-900">
                    {emptyBottleData.reduce((sum, item) => sum + item.stock, 0).toLocaleString()}
                  </p>
                </div>
              </div>
              <div className="space-y-2">
                {emptyBottleData.map((item) => (
                  <div key={item.id} className="flex items-center justify-between text-sm">
                    ...
                    <span className="font-medium text-gray-700">{item.stock.toLocaleString()}</span>
                  </div>
                ))}
              </div>
            </div>
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition-shadow duration-200">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <p className="text-xs text-gray-400 font-medium">Low Empty Bottle Branches</p>
                  <p className="text-xl font-bold text-gray-900">
                    {emptyBottleData.filter((item) => item.status === 'low').length}
                  </p>
                </div>
              </div>
              <div className="space-y-2">
